...
from NanoporeAnalysis.reference_data import CODON_TO_RESIDUE, NUCLEOTIDES, RESIDUES
...
def return_count_dict(records_input: dict | list, key: str, descending=True):
    """
    ...
    records = list(records_input.values()) if isinstance(records_input, dict) else list(records_input)
    ...
    for v in record_values:
        n_per_unique[v] += 1
    n_per_unique = dict(sorted(n_per_unique.items(), key=lambda x: x[1], reverse=descending))
    return n_per_unique
...
def translate(sequence: str, strict_dna: bool = True) -> str:
    """
...
def len_check(sequence: str, min_len: int, max_len: int) -> bool:
    """
...
def return_null_orf():
    return dict([(k, None) for k in ["ORF", "Protein", "ORF_start", "ORF_end", "ORF_strand", "Protein_length", "n"]])
...
def orf_searcher(
    ...
    if longest_only:
        longest_orfs = dict([((r["ORF_strand"], r["ORF_end"]), {"Protein_length": 0}) for r in records])
        for record in records:
            key = (record["ORF_strand"], record["ORF_end"])
            if record["Protein_length"] > longest_orfs[key]["Protein_length"]:
                longest_orfs[key] = record
        records = sorted(longest_orfs.values(), key=lambda x: x["Protein_length"], reverse=True)
...
def return_best_orf(
    ...
    candidate_orfs = orf_searcher(dna_sequence, min_length, both_strands, longest_only, strict_dna)
    if len(candidate_orfs) > 0:
        sorted_orfs = sorted(candidate_orfs, key=lambda x: x["Protein_length"], reverse=True)
